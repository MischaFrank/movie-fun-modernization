...
public class CoverCatalog {
    ...
    @Autowired
    private RedisTemplate<Long, Blob> template;
    ...
    @HystrixCommand(fallbackMethod = "getRedisCacheBlob")
    public Blob getCover(long albumId) throws IOException, URISyntaxException {
        ...
        template.opsForValue().set(albumId, coverBlob);
        return coverBlob;

    }

    public Blob getRedisCacheBlob(long albumId) throws IOException, URISyntaxException {
        return template.opsForValue().get(albumId);
    }

    public Blob buildDefaultCoverBlob(long albumId) throws IOException, URISyntaxException {
